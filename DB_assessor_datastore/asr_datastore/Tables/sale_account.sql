CREATE TABLE [asr_datastore].[sale_account] (
    ...
    CONSTRAINT [fk_sa_account] FOREIGN KEY ([account_no]) REFERENCES [asr_datastore].[account] ([account_no]) ON DELETE CASCADE,
    CONSTRAINT [fk_sa_sale] FOREIGN KEY ([recording_no]) REFERENCES [asr_datastore].[sale] ([recording_no]) ON DELETE CASCADE
);
...
GO
CREATE NONCLUSTERED INDEX [sale_account_ix_test]
    ON [asr_datastore].[sale_account]([account_no] ASC)
    INCLUDE([recording_no]);


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Accounts involved in real estate transactions.', @level0type = N'SCHEMA', @level0name = N'asr_datastore', @level1type = N'TABLE', @level1name = N'sale_account';
