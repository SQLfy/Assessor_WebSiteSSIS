CREATE TABLE [asr_spatialest].[improvement_detail] (
    ...
    CONSTRAINT [fk_id_account] FOREIGN KEY ([account_no]) REFERENCES [asr_spatialest].[account] ([account_no]),
    CONSTRAINT [fk_id_improvement] FOREIGN KEY ([account_no], [improvement_no]) REFERENCES [asr_spatialest].[improvement] ([account_no], [improvement_no]) ON DELETE CASCADE
);
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'An unique identifier for each improvement detail.', @level0type = N'SCHEMA', @level0name = N'asr_spatialest', @level1type = N'TABLE', @level1name = N'improvement_detail', @level2type = N'COLUMN', @level2name = N'improvement_detail_id';
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'The description of the addon code.', @level0type = N'SCHEMA', @level0name = N'asr_spatialest', @level1type = N'TABLE', @level1name = N'improvement_detail', @level2type = N'COLUMN', @level2name = N'addon_code_description';


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'The year the detail was put in place.', @level0type = N'SCHEMA', @level0name = N'asr_spatialest', @level1type = N'TABLE', @level1name = N'improvement_detail', @level2type = N'COLUMN', @level2name = N'detail_built_year';
