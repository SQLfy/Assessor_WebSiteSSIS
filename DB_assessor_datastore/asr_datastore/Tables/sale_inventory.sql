CREATE TABLE [asr_datastore].[sale_inventory] (
    ...
    CONSTRAINT [fk_si_account] FOREIGN KEY ([account_no]) REFERENCES [asr_datastore].[account] ([account_no]) ON DELETE CASCADE
);
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'The total square footage of the building, including all Built As groups, excluding basements and mezzanines.  Othewise know as GLA or GBA.', @level0type = N'SCHEMA', @level0name = N'asr_datastore', @level1type = N'TABLE', @level1name = N'sale_inventory', @level2type = N'COLUMN', @level2name = N'total_improvement_square_footage';
